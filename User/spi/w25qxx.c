...
{	
	...
	RCC_APB2PeriphClockCmd(SPI_CS0_RCC, ENABLE );//PORTB时钟使能 

	GPIO_InitStructure.GPIO_Pin = SPI_CS0_PIN;  // PB12 推挽 
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;  //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(SPI_CS0_GRP, &GPIO_InitStructure);
 	GPIO_SetBits(SPI_CS0_GRP,SPI_CS0_PIN);
 
        SPI_CS0_CS = 1;				//SPI FLASH不选中
	SPI2_Init();		   	//初始化SPI
...
{ 
	...
	while(1) 
	{	
		W25QXX_Read(CS_0,W25QXX_BUF,secpos*4096,4096);//读出整个扇区的内容
		...
		if(i<secremain)//需要擦除
		{
			W25QXX_Erase_Sector(CS_0,secpos);		//擦除这个扇区
...
{   
	while((W25QXX_ReadSR(CS_0)&0x01)==0x01);  		// 等待BUSY位清空
}  
//进入掉电模式
